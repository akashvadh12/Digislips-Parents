...
class ContactController extends GetxController {
  ...
  Future<void> fetchAllContacts() async {
    try {
      ...
      for (var doc in studentsSnapshot.docs) {
        ...
        if (contact['department'] != null && contact['department'].toString().isNotEmpty) {
          departmentSet.add(contact['department']);
        }
        if (contact['semester'] != null && contact['semester'].toString().isNotEmpty) {
          semesterSet.add(contact['semester']);
        }
        
        // Add parent as separate contact if parent phone exists
        if (contact['parentPhone'] != null && contact['parentPhone'].toString().isNotEmpty) {
          Map<String, dynamic> parentContact = {
            'id': '${doc.id}_parent',
            'type': 'parent',
            'fullName': '${contact['parentName']} (Parent of ${contact['fullName']})',
      ...
      contacts.sort((a, b) => a['fullName'].toString().compareTo(b['fullName'].toString()));
  ...
  void _filterContacts() {
    List<Map<String, dynamic>> filtered = allContacts.where((contact) {
      final matchesSearch = searchQuery.value.isEmpty ||
          contact['fullName'].toString().toLowerCase().contains(searchQuery.value.toLowerCase()) ||
          contact['rollNumber'].toString().toLowerCase().contains(searchQuery.value.toLowerCase()) ||
          contact['department'].toString().toLowerCase().contains(searchQuery.value.toLowerCase());
      
      final matchesDepartment = selectedDepartment.value == 'All' ||
          contact['department'] == selectedDepartment.value;
      
      final matchesSemester = selectedSemester.value == 'All' ||
  ...
  List<Map<String, dynamic>> get studentContacts => 
      filteredContacts.where((contact) => contact['type'] == 'student').toList();
